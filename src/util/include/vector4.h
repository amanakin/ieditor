...
struct Vector4 {
    Vector4<T>();
    Vector4<T>(T x, T y, T z, T t = 1);
    Vector4<T>(const Vector4<T>&)            = default;              
...
template <typename T>
inline Vector4<T>::Vector4(T x, T y, T z, T t) : 
    x(x), y(y), z(z), t(t)
{}
...
inline Vector4<T> operator+(const Vector4<T>& vector1, const Vector4<T>& vector2) {
    return Vector4<T>(vector1.x + vector2.x,
                      vector1.y + vector2.y,
                      vector1.z + vector2.z,
                      vector1.t + vector2.t);
}
